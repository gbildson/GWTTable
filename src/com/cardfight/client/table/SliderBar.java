...
public class SliderBar extends FocusPanel implements /*ResizableWidget,*/
  ...
  /**
   * Determine what the new value would have been.
   * 
   * @param event the mouse event
   */
  private void moveInDirectionOfClick(Event event) { 
    int x = DOM.eventGetClientX(event);
    double newVal = 0;
    if (x > 0) {
      double lineWidth = DOM.getElementPropertyInt(lineElement, "offsetWidth");
      double lineLeft = DOM.getAbsoluteLeft(lineElement);
      double percent = (double) (x - lineLeft) / lineWidth * 1.0;
      newVal = getTotalRange() * percent + minValue;
      //com.cardfight.client.GWTTable.logStatic(" nv: "+newVal + " lw: "+lineWidth+" lL: "+lineLeft);
    }
    double curVal = getCurrentValue();
    if ( newVal > curVal ) {
        setCurrentValue(curVal +  stepSize, true);
        //com.cardfight.client.GWTTable.logStatic(" cV: "+curVal+ " + sS: "+stepSize);
    } else if ( newVal < curVal ){
        setCurrentValue(curVal -  stepSize, true);
        //com.cardfight.client.GWTTable.logStatic(" cV: "+curVal+ " - sS: "+stepSize);
    } else {
        //com.cardfight.client.GWTTable.logStatic(" nowhere cV: "+curVal);
    }
  }
